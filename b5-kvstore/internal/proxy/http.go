...
// NewHTTPHandler builds the external REST surface (spec §2): one path per
// key, HTTP verb encodes the operation, plus a proxy-only /healthz
...
// X-Commit-Index header (so a load generator can read it with a header
// lookup instead of a JSON parse, per §2's note).
...
// writeError renders the spec §2 uniform error envelope, including
