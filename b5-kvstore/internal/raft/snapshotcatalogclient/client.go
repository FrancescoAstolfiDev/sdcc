...
// (md-week5.md §0): every reconnection attempt re-resolves fresh instead of
// reusing a cached gRPC resolver, exactly like internal/raft/grpctransport's
...
// GetLatestSnapshotInfo is deliberately fail-fast — no WaitForReady
// (md-week5.md §0): this is a cheap, frequent (every SNAPSHOT_POLL_INTERVAL_MS)
// poll, so a transient failure just means "try again next interval."
...
// into a decoded snapshotfile.File. WaitForReady(true) (md-week5.md §0): a
// node catching up should wait rather than fail-fast if the Snapshot &
